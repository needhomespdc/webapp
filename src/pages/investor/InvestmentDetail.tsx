  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-white">{investment.property?.title ?? 'Investment'}</h1>
          <p className="text-white/50 text-sm mt-1">{investment.property?.location}</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
          ...
          <p className="text-white text-lg font-bold">{formatCurrency(investment.totalAmount)}</p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
          <p className="text-white/50 text-xs mb-1">Units Held</p>
          <p className="text-white text-lg font-bold">{investment.quantity}</p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
          <p className="text-white/50 text-xs mb-1">Price per Unit</p>
          <p className="text-white text-lg font-bold">{formatCurrency(investment.pricePerUnit)}</p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
          <p className="text-white/50 text-xs mb-1">Invested On</p>
          <p className="text-white text-lg font-bold">{formatDate(investment.createdAt)}</p>
        </div>
      </div>
      ...
      {investment.milestones && investment.milestones.length > 0 && (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
          <h2 className="text-white font-semibold text-sm mb-3">Progress Timeline</h2>
            ...
            {investment.milestones.map((ms) => (
              <div key={ms.milestoneId} className="flex items-start gap-3">
                ...
                <div>
                  ...
                  {ms.completedAt && <p className="text-white/40 text-xs mt-0.5">{formatDate(ms.completedAt)}</p>}
                </div>
              </div>
            ))}
          </div>
      ...
      {investment.handoverDetails?.handoverDate && (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
          <h2 className="text-white font-semibold text-sm mb-2">Handover Details</h2>
          <p className="text-white/70 text-sm">Date: {formatDate(investment.handoverDetails.handoverDate)}</p>
          {investment.handoverDetails.notes && (
            <p className="text-white/50 text-sm mt-1">{investment.handoverDetails.notes}</p>
          )}
        </div>
      )}
